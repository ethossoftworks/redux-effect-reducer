...
const libName = "@ethossoftworks/redux-effect-reducer"
...
var newTarballs = fs.readdirSync(path.resolve(rootDir, "../build")).filter((file) => file.endsWith(".tgz"))
newTarballs.forEach((file) => {
    fs.copyFileSync(path.resolve(rootDir, `../build/${file}`), path.resolve(rootDir, `./${file}`))
